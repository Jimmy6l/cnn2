...
def parse_args():
    ...
    parser.add_argument('--clean_dir', default='data/Test', type=str, help='directory of clean test dataset')
    # 测试集名字  set68 set12 , 'Set12'
    parser.add_argument('--set_names', default=['0.4'], help='directory of test dataset')
    parser.add_argument('--clean_names', default=['0.4'], help='directory of test dataset')
    # 噪声水平  默认25
    parser.add_argument('--sigma', default=25, type=int, help='noise level')
    # 模型位置  models/DnCNN_sigma25
    parser.add_argument('--model_dir', default=os.path.join('models', 'DsCNN_abl1_sigma28'), help='directory of the model')
    # 模型名字 默认model_001.pth
    parser.add_argument('--model_name', default='202212model_001.pth', type=str, help='the model name')
    # 结果位置  results
    parser.add_argument('--result_dir', default='results/DsCNN_abl1_loss', type=str, help='directory of test dataset')
    # 保存结果 1保存 0不保存
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                x = np.array(imread(os.path.join(args.clean_dir, set_cur, im),0), dtype=np.float32)/255.0
                ...
                y = np.array(imread(os.path.join(args.set_dir, set_cur, im),0), dtype=np.float32)/255.0   # No Add Gaussian noise without clipping
                ...
                if args.save_result:
                    ...
                    save_result(x_, path=os.path.join(args.result_dir, set_cur, name+''+ext))  # save the denoised image
        ...
        if args.save_result:
            ...
            save_result(np.hstack((psnrs, ssims,times)), path=os.path.join(args.result_dir, set_cur, 'results.txt'))
        # 打印日志，调用log函数
        log('Datset: {0:10s} \n  PSNR = {1:2.2f}dB, SSIM = {2:1.4f}, time = {3:.8f}'.format(set_cur, psnr_avg, ssim_avg,time_avg))
