...
def parse_args():
    ...
    # 增加参数
    # 测试集  data/Test
    parser.add_argument('--clean_dir', default='data/test', type=str, help='directory of test dataset')
    ...
    parser.add_argument('--model_dir', default=os.path.join('models', 'DsCNN_5_sigma28'), help='directory of the model')
    # 模型名字 默认model_001.pth
    parser.add_argument('--model_name', default='202212model_006.pth', type=str, help='the model name')
    # 结果位置  results
    parser.add_argument('--result_dir', default='results/DsCNN_loss', type=str, help='directory of test dataset')
    # 保存结果 1保存 0不保存
    ...
    for set_cur in args.set_names:
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                x = np.array(imread(os.path.join(args.clean_dir, set_cur, im)), dtype=np.float32)/255.0
                ...
                y = np.array(imread(os.path.join(args.set_dir, set_cur, im)), dtype=np.float32)/255.0   # No Add Gaussian noise without clipping
